...
OUTPUT_DIR  = r"C:\Users\anshu\Documents\Georgia Tech 26\Research\PALMO\Results Hyperparameterization\Cd"
...
def plot_arch_top(df, path):
    ...
    ax.bar(range(len(top)),top["r2"],color="tomato",edgecolor="black",lw=0.5)
...
def plot_training_curve(train_losses, path):
    ...
    ax.plot(train_losses,lw=1.5,color="tomato",label="Train Loss")
...
def plot_scatter(y_pred, y_test, path):
    ...
    ax.scatter(y_test,y_pred,s=1,alpha=0.2,color="tomato")
...
def plot_error_distribution(y_pred, y_test, path):
    ...
    ax.hist(errors,bins=80,color="tomato",edgecolor="none",alpha=0.85)
...
def plot_per_airfoil(y_pred, y_test, test_df, path):
    ...
    ax2.bar(labels,maes,color="tomato",edgecolor="black",lw=0.5)
