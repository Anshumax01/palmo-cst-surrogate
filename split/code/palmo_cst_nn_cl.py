...
palmo_cst_nn_cl.py
==================
PALMO CST surrogate — Cl ONLY
Full 200x200x200 dedicated to lift coefficient.
...
OUTPUT_DIR  = r"C:\Users\anshu\Documents\Georgia Tech 26\Research\PALMO\Results CST NN Split\Cl"
...
def load_data():
    ...
    scaler_y=StandardScaler(); y_train_scaled=scaler_y.fit_transform(y_train).ravel()
...
def train_model(X_train, y_train_scaled):
    ...
    model=MLPRegressor(hidden_layer_sizes=HIDDEN_LAYERS,activation="relu",solver="adam",
                       ...
                       max_iter=1,warm_start=True,tol=0.0,n_iter_no_change=MAX_ITER,
    ...
    for epoch in range(1,MAX_ITER+1):
        ...
        if val_loss<best_val-1e-8:
            best_val=val_loss; best_coefs=[c.copy() for c in model.coefs_]
            best_intercepts=[b.copy() for b in model.intercepts_]; no_improve=0
        else: no_improve+=1
        if epoch%50==0 or epoch==1:
            print(f"  Epoch {epoch:4d}/{MAX_ITER}  train={tr_loss:.6f}  val={val_loss:.6f}")
        if no_improve>=PATIENCE:
            print(f"\n  Early stop at epoch {epoch}  (best val={best_val:.6f})"); break
    model.coefs_=best_coefs; model.intercepts_=best_intercepts
...
def plot_scatter(y_pred, y_test, path):
    r2=r2_score(y_test,y_pred); lo=min(y_test.min(),y_pred.min()); hi=max(y_test.max(),y_pred.max())
...
def plot_per_airfoil(y_pred, y_test, test_df, path):
    ...
    for af in airfoils:
        ...
        r2s.append(r2_score(y_test[mask],y_pred[mask])); maes.append(mean_absolute_error(y_test[mask],y_pred[mask])); labels.append(f"NACA\n{af}")
...
def main():
    ...
    plot_loss_summary(tr_loss,val_loss,y_pred,y_test,
                      path_curve=out("cl_training_curve.png"),path_summary=out("cl_loss_summary.png"))
    plot_scatter(y_pred,y_test,               path=out("cl_predictions_scatter.png"))
    plot_error_distribution(y_pred,y_test,    path=out("cl_error_distribution.png"))
    plot_aoa_curve(y_pred,y_test,test_df,     path=out("cl_aoa_curve.png"))
    plot_multi_mach(y_pred,y_test,test_df,    path=out("cl_multi_mach.png"))
    plot_error_vs_aoa(y_pred,y_test,test_df,  path=out("cl_error_vs_aoa.png"))
    plot_per_airfoil(y_pred,y_test,test_df,   path=out("cl_per_airfoil.png"))
    save_predictions(y_pred,y_test,test_df,   path=out("cl_predictions.csv"))
