...
OUTPUT_DIR  = r"C:\Users\anshu\Documents\Georgia Tech 26\Research\PALMO\Results Baseline Split\Cd"
...
def load_data():
    ...
    # Cd gets its OWN scaler — no cross-contamination with Cl or Cm
...
def plot_drag_polar(y_pred, y_test, test_df, path,
                    mach=0.25, reynolds=1_000_000):
    airfoils = sorted(test_df[AIRFOIL_COL].unique())
    fig, axes = plt.subplots(1, len(airfoils),
                             figsize=(5*len(airfoils), 5), sharey=True)
    ...
    for ax, af in zip(axes, airfoils):
        mask = ((np.abs(test_df["Mach"].values - mach) < 1e-4) &
                ...
                (test_df[AIRFOIL_COL].values == af))
        if mask.sum() == 0: continue
        cl_cfd = test_df.loc[mask, "Cl"].values
        cd_cfd = y_test[mask]
        cd_nn  = y_pred[mask]
        s = np.argsort(cl_cfd)
        ax.plot(cd_cfd[s], cl_cfd[s], "k-",  lw=2.0, label="CFD")
        ax.plot(cd_nn[s],  cl_cfd[s], "gs",  ms=5,
                markerfacecolor="none", markeredgewidth=1.2, label="NN")
        ax.set_xlabel("$c_d$")
        ax.set_title(f"NACA {af}\nM={mach}, Re={reynolds:.0e}")
        ax.legend(fontsize=8)
    axes[0].set_ylabel("$c_l$")
    plt.suptitle("PALMO Cd — Drag Polars", fontsize=13, y=1.02)
    plt.tight_layout()
    plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
    print(f"Saved: {path}")
...
def main():
    ...
    plot_drag_polar(y_pred, y_test, test_df,   path=out("cd_drag_polar.png"))
