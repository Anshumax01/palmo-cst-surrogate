"""Palmo_v2.py
Extends the NN inverse-design pipeline with two new capabilities:

  Mode A  –  Arbitrary targets
             Prescribe any Cl / Cd / Cm values directly; the optimizer
             finds a shape that matches them.

  Mode B  –  Drag minimisation
             Initialise from a reference airfoil, lock Cl (and optionally Cm)
             to the airfoil's predicted values, and minimise Cd instead of
             matching it to a prescribed target.

Structure mirrors Palmo.py (Sections 1-6) with additions marked NEW.
"""
...
# ---- LAM suite (boss's code, untouched) ----
...
## SECTION 4 – NNProbabilisticDesigner  (unchanged base + NEW minimize_cd)
...
if __name__ == '__main__':
    ...
    # ── NACA 3415 PCA vector (same as Palmo.py) ───────────────────────────────
    naca3415_pca = (
        torch.tensor(
    ...
    # ==========================================================================
    # ── Choose a run mode ─────────────────────────────────────────────────────
    # Set RUN_MODE to one of:
    #   'original'    – match NACA 3415 predicted Cl/Cd/Cm  (Palmo.py behaviour)
    #   'arbitrary'   – match user-prescribed Cl/Cd/Cm targets
    #   'minimize_cd' – match NACA 3415 Cl & Cm; minimise Cd
    # ==========================================================================
    ...
    # --------------------------------------------------------------------------
        ...
        # ── NEW: prescribe any targets you like ──────────────────────────────
        # Adjust these values freely; the optimizer will search for a shape
        # that achieves them (within the surrogate's fidelity).
        targets = build_arbitrary_targets(
    ...
    # --------------------------------------------------------------------------
        ...
        # ── NEW: Drag minimisation ────────────────────────────────────────────
        # Step 1 – evaluate the reference airfoil to get its Cl and Cm targets
